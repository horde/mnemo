...
switch ($actionID) {
    ...
    case 'save_memo':
        ...
        $memo_tags = Util::getFormData('memo_tags');
...
if (!$view->modify || !$view->passphrase) {
    $injector->getInstance('Horde_Core_Factory_Imple')
        ->create('Mnemo_Ajax_Imple_TagAutoCompleter', ['id' => 'memo_tags']);
    $view->body = $memo_body;
    $view->count = sprintf(
        _("%s characters"),
        '<span id="mnemo-count">'
            . Horde_String::length(str_replace(["\r", "\n"], '', $memo_body))
            . '</span>'
    );
    ...
    if ($memo_id
        ...
        $view->delete = Horde::url('memo.php')->add([
            'memo' => $memo_id,
